...
def test_delete_student():
    ...
    # Delete first and last students
...
# 2nd parameter of this function is useless
...
def add_student_command_ui(student_list, command_params):
    param_list = command_params.split(',')
    if len(param_list) != 3:
        raise ValueError("Wrong parameters entered!")
    student_id = param_list[0].strip()
    if find_by_id(student_list, student_id) >= 0:
        raise ValueError("Duplicate student id!")
    student_name = param_list[1].strip()
    student_grade = int(param_list[2].strip())
    student_list.append(create_student(student_id, student_name, student_grade))

def delete_student_comand_ui(student_list, command_params):
    id_list = command_params.split(',')
    if '-' in id_list[0]:
        range_list = id_list[0].split('-')
        r1 = int(range_list[0].strip())
        r2 = int(range_list[1].strip())
        id_list.clear()
...
def test_split_command():
    for cmd in ['add 1, Pop Ionescu, 5', '  aDd 1, Pop Ionescu, 5', 'Add       1, Pop Ionescu, 5   ']:
...
def start_command_ui():
    ...
    # 2. separate command and its param
    # 3. call a separate function for each valid command
    done = False

    student_list = []
    test_init(student_list)
    command_dict = {'delete': delete_student_comand_ui, 'show': show_all_students_ui, 'add': add_student_command_ui}
    while not done:
